...
export class DocumentRpcController {
  ...
  async create(
    ...
    try {
      ...
      this.aiClient.emit(AI_DOCUMENT_EVENTS.DOCUMENT_CREATED, {
        ...
        tags: doc.tags,
        linkedAppointmentId: doc.linkedAppointmentId?.toString(),
        linkedProviderId: doc.linkedProviderId?.toString(),
      });
  ...
  async update(
    ...
    try {
      return await this.documentService.update(data.documentId, data.dto);
    } catch (err) {
      this.handleRpcError(err);
    }
  }

  @MessagePattern(DOCUMENT_PATTERNS.UPDATE_EXTRACTION_STATUS)
  async updateExtractionStatus(
    ...
    try {
      return await this.documentService.updateExtractionStatus(
        data.documentId,
