...
export class ProfileService {
  constructor(
    ...
    private readonly profileModel: Model<ProviderProfileDocument>,
  ) {}
  ...
  async list(query: any) {
    const features = new ApiFeatureService(query, this.profileModel)
      .filter()
      .sort()
      .select()
      .paginate();
  ...
  async verifyCredential(
    ...
    if (!profile)
      throw new NotFoundException('Profile or credential not found');
    return profile;
  }
}
