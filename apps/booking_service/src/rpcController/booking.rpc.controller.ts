...
export class BookingRpcController {
  ...
  async getMyAppointments(
    @Payload() data: { userId: string; role: string; query: any },
  ) {
    try {
  ...
  async releaseSlot(@Payload() data: { slotId: string; patientId: string }) {
    try {
      return await this.availabilityService.releaseSlot(
        data.slotId,
        data.patientId,
      );
    } catch (err) {
  ...
  async createSchedule(
    @Payload() data: { dto: CreateProviderScheduleDto; providerId: string },
  ) {
    try {
  ...
  async updateSchedule(
    @Payload() data: { providerId: string; dto: UpdateProviderScheduleDto },
  ) {
    try {
  ...
  async getMySchedule(@Payload() data: { providerId: string }) {
    try {
      return await this.availabilityService.scheduleModel.findOne({
        providerId: data.providerId,
      });
    } catch (err) {
      this.handleRpcError(err);
