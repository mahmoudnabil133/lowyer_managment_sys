...
export class TimeSlot {
    ...
    @Prop({ type: String, required: true, enum: SlotStatus, default: SlotStatus.AVAILABLE, index: true })
    status: SlotStatus;
...
TimeSlotSchema.index({ heldExpireDate: 1 }, { expireAfterSeconds: 0, sparse: true });

TimeSlotSchema.virtual('isAvailable').get(function () {
    return (
        this.status === SlotStatus.AVAILABLE &&
        this.bookedCount < this.maxBookings
    )
});