...
export class RagService implements OnModuleInit, OnModuleDestroy {
  ...
  async similaritySearch(
    ...
    if (filters?.appointmentId) {
      preFilter['metadata.appointmentId'] = { $eq: filters.appointmentId };
    }
    if (filters?.providerId) {
      preFilter['metadata.providerId'] = { $eq: filters.providerId };
    }
    if (filters?.category) {
      preFilter['metadata.category'] = { $eq: filters.category };
    }
    if (filters?.documentIds?.length) {
      preFilter['metadata.documentId'] = { $in: filters.documentIds };
    }
