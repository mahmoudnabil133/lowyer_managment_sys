...
export class UserService {
  ...
  async createUser(createUserDto: CreateUserDto) {
    ...
    const hashed_pass: string = await bcrypt.hash(createUserDto.password, 12);
  ...
  async findAllUsers(query: ApiQueryDto): Promise<PaginatedResponse<User>> {
    const results = await new ApiFeatureService<User>(query, this.userModel)
      .filter()
      .sort()
      .select()
      .paginate()
  ...
  async findByResetCode(resetCode: string): Promise<UserDocument | null> {
    const user = await this.userModel.findOne({
