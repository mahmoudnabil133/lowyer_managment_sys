...
export class AuthRpcController {
    ...
    async validatePasswordResetCode(@Payload() data: { code: string; password: string }) {
        try {
            return await this.authService.validatePasswordResetCode(data.code, data.password);
        } catch (err) {
            this.handleRpcError(err);
        }
    }
