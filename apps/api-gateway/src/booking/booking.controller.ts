import { Controller, Post, Body, Get, Param, Patch, Query, HttpCode, HttpStatus, Delete, UseGuards } from '@nestjs/common';
...
export class BookingController {
    ...
    async book(@Body() dto: BookAppointmentDto, @CurrentUser() user: AuthUserDto) {
        console.log(user);
    ...
    async cancel(@Body() dto: CancelAppointmentDto, @CurrentUser() user: AuthUserDto) {
    ...
    @HttpCode(HttpStatus.OK)
    async reschedule(@Body() dto: RescheduleAppointmentDto, @CurrentUser() user: AuthUserDto) {
        const role = user.role === 'user' ? 'patient' : user.role;
        return this.bookingService.reschedule(dto, user.userId, role);
    }

    @Patch('appointments/status')
    async updateStatus(@Body() dto: UpdateAppointmentStatusDto, @CurrentUser() user: AuthUserDto) {
        const role = user.role === 'user' ? 'patient' : user.role;
        return this.bookingService.updateStatus(dto, user.userId, role);
    }

    @Patch('appointments/provider-notes')
    async addProviderNotes(@Body() dto: ProviderNotesDto, @CurrentUser() user: AuthUserDto) {
        return this.bookingService.addProviderNotes(dto);
    }

    @Get('appointments/my')
    async getMyAppointments(@CurrentUser() user: AuthUserDto, @Query() query: any) {
    ...
    async getAvailabilityRange(@Query() dto: GetAvailableRangeDto) {
        return this.bookingService.getAvailabilityRange(dto.providerId, dto.fromDate, dto.toDate);
    }

    @Post('availability/hold/:slotId')
    @HttpCode(HttpStatus.OK)
    async holdSlot(@Param('slotId') slotId: string, @CurrentUser() user: AuthUserDto) {
        return this.bookingService.holdSlot(slotId, user.userId);
    }

    @Delete('availability/hold/:slotId')
    @HttpCode(HttpStatus.OK)
    async releaseSlot(@Param('slotId') slotId: string, @CurrentUser() user: AuthUserDto) {
    ...
    @Post('schedules')
    async createSchedule(@Body() dto: CreateProviderScheduleDto, @CurrentUser() user: AuthUserDto) {
        // only provider/admin
    ...
    async updateSchedule(@Param('providerId') providerId: string, @Body() dto: any) {
        return this.bookingService.updateSchedule(providerId, dto);
    }

    @Post('schedules/generate-slots')
    async generateSlots(@CurrentUser() user: AuthUserDto, @Query('days') days = 30) {
